...
export default (props => {
  let {
    ...
    chartData,
    limits
  } = props; // Generate default structure to handle null props
  ...
  if (cd.plotPoints.length > 0) {
    const {
      plotPoints: dataArray
    } = cd; // Iterate over plot points and check if they are in the correct format for rendering i.e. [{ x: 1, y: 1 }]
  ...
  const calculateLimits = () => {
    ...
    const upperLimit = Object.assign({}, ...[limitObject, {
      ...
      data: lim.upper.values && Array.isArray(lim.upper.values) && lim.upper.values.length > 0 && lim.upper.values.length >= cd.plotPoints.length ? lim.upper.values : calculationData.map(d => d.mean + 2.66 * averageAMR)
    }]);
    ...
    const lowerLimit = Object.assign({}, ...[limitObject, {
      ...
      data: lim.lower.values && Array.isArray(lim.lower.values) && lim.lower.values.length > 0 && lim.lower.values.length >= cd.plotPoints.length ? lim.lower.values : calculationData.map(d => d.mean - 2.66 * averageAMR)
    }]);

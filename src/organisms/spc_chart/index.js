...
export default props => {
  ...
  let co = Object.assign(
    { title: null, xAxis: {}, yAxis: {}, legend: {} },
    chartOptions
  );
  let cd = Object.assign({ description: null, plotPoints: [] }, chartData);
  let lim = Object.assign(
    { rules: [], upper: {}, mean: {}, lower: {} },
    limits
  );
  ...
  if (cd.plotPoints.length > 0) {
    ...
    dataArray.forEach(plotPoint => {
      ["x", "y"].forEach(axis => {
        if (
          Object.keys(plotPoint).includes(axis) &&
          plotPoint[axis] !== null && plotPoint[axis] !== undefined &&
          typeof plotPoint[axis] === "number"
        )
          axisCount[axis]++;
      });
    });
    if (
      axisCount.x !== plotPointArrayLength ||
      axisCount.y !== plotPointArrayLength
    )
  ...
  const calculationData = cd.plotPoints.map((pp, idx) => ({
    data: pp.y,
    amr:
      cd.plotPoints.length - 1 === idx
        ? null
        : Math.abs(pp.y - cd.plotPoints[idx + 1].y),
    mean:
      cd.plotPoints.map(pp => pp.y).reduce((a, b) => a + b, 0) /
      cd.plotPoints.length
  }));
  ...
  const createLimitObject = (limit, type) => {
    const baseObject = {
      ...
      pointStart:
        cd.plotPoints &&
        Array.isArray(cd.plotPoints) &&
        cd.plotPoints.length > 0
          ? cd.plotPoints[0].x
          : 0,
      color: null,
      data: [],
    ...
    const createLimitData = () => {
      if (
        limit.values &&
        Array.isArray(limit.values) &&
        limit.values.length > 0 &&
        limit.values.length >= cd.plotPoints.length
      )
        return limit.values;
      const amrList = calculationData.map(cd => cd.amr).filter(Boolean);
      if (amrList.length < 1) return limit.values;
      const amr =
        amrList.reduce((a, b) => (b += a)) /
        calculationData.map(cd => cd.amr).filter(Boolean).length;
      ...
      lim.rules.forEach(r => {
        switch (r.trim().toLowerCase()) {
          case "western electric":
            if (p.y > upperLimitValue || p.y < lowerLimitValue)
              rulesBreached.WesternElectric.push(p.x);
            ruleSet.push({
              color:
                p.y > upperLimitValue
                  ? lim.upper.anomalyColor || "red"
                  : p.y < lowerLimitValue
                  ? lim.lower.anomalyColor || "red"
                  : null
            });
            break;
          default:
  ...
  const options = {
    ...
    tooltip: {
      ...
      formatter: function(t) {
        ...
        const body = t.options.pointFormat
          .replace("{point.color}", this.color)
          .replace("{series.name}", this.series.name)
          .replace("{point.y}", this.y);
        const hasBreachedRule = Object.keys(rulesBreached).some(r =>
          rulesBreached[r].includes(this.key)
        );
        let rule = `${
          hasBreachedRule ? '<div style="margin-bottom:1rem;">' : "<div>"
        }
                ${Object.keys(rulesBreached).map(r =>
                  rulesBreached[r].includes(this.key)
                    ? `<div style="font-weight:bold">${r
                        .split(/(?=[A-Z])/)
                        .join(" ")} Rule Broken</div>`
                    : null
                )}
                </div>`;
